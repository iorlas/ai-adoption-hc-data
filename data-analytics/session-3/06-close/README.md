...
> **Before tomorrow, run one of today's two checks on a real report of your
> own.** Either the definition check (does this measure match what we say the
> measure means?) or the cross-check (does another report claim the same measure
> and get a different number?).

Use the shared dataset if you would rather not point it at real work — it still
teaches the loop.
