# 0 — Opening (15 min)

*No tools. Nothing to run yet.*

## Yesterday's practice ask

Monday's close asked everyone to run one of the two checks — the definition
check or the cross-check — on a real report of your own.

**"I did not get to it" is a completely fine answer.** Most people will not have,
and the ones who did are not the point.

If you did try it, three things worth hearing:

- Did you find a disagreement?
- Was the answer usable, or did it need context Claude did not have?
- What did you have to tell it that you did not have to tell it on Monday?

That last one is the bridge into today. **On your own work, the context is the
work.** The pipeline in `adf/` has no documentation, and the first thing you do
this morning is build the context that lets anyone — you or Claude — reason
about it.

## How today is shaped

Three passes over one pipeline: **understand it, document it, find where it
breaks.** Then a shorter piece on asking better questions of data, and thirty
minutes on machine learning that you watch rather than run.

## Worth being clear about again

- **Everything is synthetic.** The pipeline is a teaching artifact, not anyone's
  real pipeline.
- **Nothing today connects to Azure.** No factory, no workspace, no credentials.
  The pipeline is five JSON files on your laptop.
- **We are not proposing you stop using the ADF interface.** In your words:
  *"it's not that we want to bypass using ADF, it's that we want to make
  ourselves using ADF easier."*

## Before we start

You need Claude open and signed in, and this folder on your machine — including
yesterday's `CLAUDE.md` with your definitions in it, and
`docs/data-quality-rules.md`.

**Power BI is not needed today.**

If anything misbehaves, [`quirks.md`](../../quirks.md) has the fix.
