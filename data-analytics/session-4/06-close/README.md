...
- Lauren's line from the August call is the standard to judge against: *"I'd
...
- **Databricks and machine learning, properly** — hands-on, on a real workspace,
  for whoever wants it. Today was thirty minutes and a taster.
- **Git** — including the Power BI collaboration and merge-conflict pain Lucie
  described. That is a real problem and it deserves its own session rather than
  fifteen minutes bolted onto something else. Roughly an hour and a half.
- **Genie hands-on**, if the comparison on Monday made it look worth having.
- **MCP** — connecting Claude directly to a data source rather than to files.
  We mentioned this on the August call. It needs an IT conversation that has not
